...
- **Client UI (sidebar):** `shared/clientNav.ts` drives which nav links appear by `platformRole` (and legacy `role`): platform staff see operator tools; company members do not see billing/officer registry/SLA admin, etc.; `company_admin` sees renewal workflows and company admin; `finance_admin` / `hr_admin` see payroll and PDF reports; `platformRole: client` **without** an active company workspace gets a minimal shell (dashboard, client portal, contracts, alerts, hub, preferences). Optional items (analytics, compliance, marketplace, recruitment, quotations) can be hidden per browser via `/preferences`. **Company hub** (`/company/hub`) maps departments (sales, marketing, HR, …) to existing modules.
- **Pre-company workspace (client):** After membership resolution settles to **no companies**, non-operator/non-portal users see a setup-first dashboard and a reduced nav (`PRE_COMPANY_NAV_HREFS` / `shouldUsePreRegistrationShell`). **Invariant:** pre-company mode only applies after `companies.myCompanies` has loaded; loading and platform/operator/portal shells are excluded. See [architecture/workspace-mode.md](./architecture/workspace-mode.md).

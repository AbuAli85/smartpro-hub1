...
import {
  HR_LETTERS,
  memberHasHrLetterPermission,
} from "@shared/hrLetterPermissions";
import {
...
function withSanitizedBodies<
  T extends { bodyEn: string | null; bodyAr: string | null },
>(row: T): T {
  return {
    ...row,
...
const generateInputBase = z.object({
  ...
  recipientPreset: z
    .enum(["twimc", "bank", "embassy", "ministry", "custom"])
    .optional(),
...
export const hrLettersRouter = router({
  ...
  listSignatories: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return db
        ...
        .where(
          and(
            eq(companySignatories.companyId, m.companyId),
            eq(companySignatories.isActive, true)
          )
        )
  ...
  createSignatory: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (
        !mem ||
        !memberHasHrLetterPermission(mem, HR_LETTERS.SIGNATORIES_MANAGE)
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "You do not have permission to manage signatories.",
        });
      }
      const db = await getDb();
      ...
      if (input.isDefault) {
        await db
          .update(companySignatories)
          .set({ isDefault: false })
          .where(eq(companySignatories.companyId, membership.companyId));
      }
  ...
  validateReadiness: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (
        !employee ||
        !company ||
        employee.companyId !== membership.companyId
      ) {
      ...
      if (db) {
        const [s] = await db
          ...
          .where(
            and(
              eq(companySignatories.id, input.signatoryId),
              eq(companySignatories.companyId, membership.companyId)
            )
          )
          .limit(1);
        signatoryRow = s ?? null;
      ...
      const purposeMerged = mergeLetterPurpose(
        input.letterType as OfficialLetterType,
        input.purpose,
        fields
      );
      let issuedTo = input.issuedTo?.trim() ?? "";
      if (input.recipientPreset === "twimc")
        issuedTo = "To Whom It May Concern";
      const issueDate = fields.issueDate?.trim()
        ? new Date(fields.issueDate)
        : new Date();
  ...
  previewLetter: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!mem || !memberHasHrLetterPermission(mem, HR_LETTERS.READ)) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "No permission to preview letters.",
        });
      }
      const employee = await getEmployeeById(input.employeeId);
      const company = await getCompanyById(membership.companyId);
      ...
      if (!vr.ok)
        return { ok: false as const, missing: vr.missing, preview: null };
      ...
      const bodies = applyLanguageMode(
        input.language,
        rendered.bodyEn,
        rendered.bodyAr
      );
  ...
  generateLetter: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      requireNotAuditor(
        membership.role,
        "External Auditors cannot generate letters."
      );
      const fullMember = await getUserCompanyById(
        ctx.user.id,
        membership.companyId
      );
      if (
        !fullMember?.member ||
        !memberHasHrLetterPermission(fullMember.member, HR_LETTERS.ISSUE)
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "No permission to issue official letters.",
        });
      }
      if (
        !canIssueSensitiveLetter(
        ...
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "No permission to issue this sensitive letter type.",
        });
      }
      ...
      if (!signatoryRow) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Select a valid signatory for this company.",
        });
      }
      ...
      const insertId =
        (insertResult[0] as { insertId?: number })?.insertId ?? null;
      const savedRows = insertId
        ? await db
            .select()
            .from(hrLetters)
            .where(eq(hrLetters.id, insertId))
            .limit(1)
        : [];
  ...
  sendLetterByEmail: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      requireNotAuditor(
        membership.role,
        "External Auditors cannot send letters."
      );
      ...
      if (letter.emailSentAt) {
        const secondsSinceLast =
          (Date.now() - new Date(letter.emailSentAt).getTime()) / 1000;
  ...
  deleteLetter: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      requireNotAuditor(
        membership.role,
        "External Auditors cannot delete letters."
      );
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      await db

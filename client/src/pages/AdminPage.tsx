...
export default function AdminPage() {
  ...
  const { data: allSettings } = trpc.analytics.getSettings.useQuery(
    { category: undefined },
    { enabled: user != null && canAccessGlobalAdminProcedures(user) },
  );
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <Tabs defaultValue="users">
        <TabsList>
          <TabsTrigger value="users" className="gap-1"><UserCog size={13} />Users</TabsTrigger>
          <TabsTrigger value="companies">Companies ({companies?.length ?? 0})</TabsTrigger>
          <TabsTrigger value="audit">Audit Logs</TabsTrigger>
          <TabsTrigger value="config">System Config</TabsTrigger>
        </TabsList>

        {/* ── Users Tab — identity & access operations console ── */}
        <TabsContent value="users" className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Platform-wide identity, linked providers, global roles, and workspace memberships — authoritative data from identity tables (not legacy
            <code className="mx-1 text-xs">users.platformRole</code>
            labels).
          </p>
          <AdminUsersConsole />
        </TabsContent>

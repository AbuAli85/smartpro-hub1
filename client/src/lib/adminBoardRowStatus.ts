...
export function getAdminBoardRowStatusPresentation(status: string): {
  ...
  const map: Record<string, { label: string; className: string }> = {
    ...
    // Completed: shift met the 80% worked threshold.
    completed: { label: "Completed", className: "border-emerald-300 text-emerald-700 bg-emerald-50" },
    // Early checkout: employee left before the completion threshold.
    early_checkout: { label: "Checked out early", className: "border-orange-300 text-orange-700 bg-orange-50" },
    // Legacy fallback for records without shift time context.
    checked_out: { label: "Checked out", className: "border-gray-300 text-gray-700 bg-gray-50" },
  };

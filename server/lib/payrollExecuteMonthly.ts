...
function countExpectedWorkdaysInMonth(
  emp: { userId: number | null; hireDate: Date | null },
  ...
  for (let d = 1; d <= lastDay; d++) {
    ...
    if (hire) {
      const hd = new Date(hire.getFullYear(), hire.getMonth(), hire.getDate());
      const cur = new Date(day.getFullYear(), day.getMonth(), day.getDate());
      if (cur < hd) continue;
    }
...
export async function executeMonthlyPayroll(db: PayrollDb, params: { companyId: number; month: number; year: number; actorUserId: number }) {
  ...
  const empList = await db
    ...
    .where(
      and(
        ...
        or(isNull(employees.hireDate), lte(employees.hireDate, monthStart))
      )
    );
  ...
  if (existingRun) {
    ...
    const oldLines = await db.select().from(payrollLineItems).where(eq(payrollLineItems.payrollRunId, runId));
    ...
    await db.delete(payrollLineItems).where(eq(payrollLineItems.payrollRunId, runId));
  ...
  for (const emp of empList) {
    const cfg = pickSalaryConfigForPeriod(salaryConfigs, emp.id, monthStart, monthEnd);
  ...
  const [finalRun] = await db.select().from(payrollRuns).where(eq(payrollRuns.id, runId)).limit(1);

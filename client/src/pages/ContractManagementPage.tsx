...
export default function ContractManagementPage() {
  ...
  const stats = useMemo(() => ({
    ...
    active:   contracts.filter((r) => r.status === "active").length,
    draft:    contracts.filter((r) => r.status === "draft").length,
    expiring: contracts.filter((r) => {
      const d = daysUntil(r.expiryDate);
      return d !== null && d >= 0 && d <= 30 && r.status === "active";
    }).length,
    expired: contracts.filter((r) => r.status === "expired").length,
  }), [contracts]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        {/* Stats */}
        <div className={`grid grid-cols-2 sm:grid-cols-5 gap-3 ${isFetching && !isLoading ? "opacity-75" : ""}`}>
          {[
            { label: "Total",         value: stats.total,    icon: <FileText   className="h-4 w-4" />,                         cls: "" },
            { label: "Active",        value: stats.active,   icon: <CheckCircle2 className="h-4 w-4 text-emerald-500" />,      cls: "" },
            { label: "Draft",         value: stats.draft,    icon: <Building2  className="h-4 w-4 text-zinc-400" />,           cls: "" },
            { label: "Expiring ≤30d", value: stats.expiring, icon: <Clock      className="h-4 w-4 text-amber-500" />,          cls: "" },
            { label: "Expired",       value: stats.expired,  icon: <AlertCircle className="h-4 w-4 text-red-500" />,           cls: "" },
          ].map((s) => (

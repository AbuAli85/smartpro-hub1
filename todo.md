...
- [x] **SYNC-FSC-WPS** GitHub sync — pulled 2 commits (merge + WPS quality scope on FinancialSummaryCard). No new migrations. `tsc --noEmit` exits 0. FinancialSummaryCard test suite expanded to 6 tests (added `wpsQualityScope` badge tests for `period` and `company_fallback`), all pass. 7 pre-existing failures unchanged.
- [x] **SYNC-SECURITY-BATCH** GitHub sync — pulled 26 commits (security hardening: CORS, CSP, rate limiting, OAuth redirect validation, WhatsApp webhook, payment webhook, service worker origin check; UI: ExcelJS refactor for EmployeeImport/HRAttendance/SurveyAdminResponses, TaskAssignDialog refactor, Chart component improvements, HREmployeesPage/HRAttendancePage enhancements; CI: removed npm audit, added pnpm-only audit). Fixed TS errors: installed exceljs, changed 3 `import ExcelJS from "exceljs"` to `import * as ExcelJS from "exceljs"`. No new migrations. `tsc --noEmit` exits 0. 33 new/updated tests pass. 7 pre-existing failures unchanged.

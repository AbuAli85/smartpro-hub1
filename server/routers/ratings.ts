...
async function recomputeOfficeScore(db: NonNullable<Awaited<ReturnType<typeof getDb>>>, officeId: number) {
  const [agg] = await db
    ...
    .from(sanadRatings)
    .where(and(eq(sanadRatings.officeId, officeId), eq(sanadRatings.isPublished, true)));
...
export const ratingsRouter = router({
  ...
  submitRating: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existing] = await db
        ...
        .from(sanadRatings)
        .where(and(eq(sanadRatings.officeId, input.officeId), eq(sanadRatings.companyId, companyId)));
      if (existing) throw new TRPCError({ code: "CONFLICT", message: "Your company has already reviewed this centre" });

      // Check if linked to a verified completed service request
      let isVerified = false;
      if (input.serviceRequestId) {
        const [req] = await db
          .select({ id: sanadServiceRequests.id, status: sanadServiceRequests.status })
      ...
      const [inserted] = await db.insert(sanadRatings).values({
        ...
        qualityRating: input.qualityRating ? clampRating(input.qualityRating) : null,
        communicationRating: input.communicationRating ? clampRating(input.communicationRating) : null,
  ...
  getOfficeRatings: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input.verifiedOnly) conditions.push(eq(sanadRatings.isVerified, true));
      ...
      const [aggregate] = await db
        ...
        .from(sanadRatings)
        .where(and(eq(sanadRatings.officeId, input.officeId), eq(sanadRatings.isPublished, true)));
      ...
      let replies: Array<{
        ratingId: number; id: number; replyBody: string;
        createdAt: Date; replierName: string | null;
      }> = [];
      if (ratingIds.length) {
        replies = await db
          .select({
          ...
          .where(sql`${sanadRatingReplies.ratingId} IN (${sql.join(ratingIds.map(id => sql`${id}`), sql`, `)})`);
      }

      const repliesByRating = replies.reduce<Record<number, typeof replies>>((acc, r) => {
      ...
      return {
        ratings: ratings.map(r => ({ ...r, replies: repliesByRating[r.id] ?? [] })),
        total: Number(aggregate.total),
        aggregate: aggregate.total
          ? {
              avgOverall: aggregate.avgOverall ? Math.round(Number(aggregate.avgOverall) * 10) / 10 : null,
              avgSpeed: aggregate.avgSpeed ? Math.round(Number(aggregate.avgSpeed) * 10) / 10 : null,
              avgQuality: aggregate.avgQuality ? Math.round(Number(aggregate.avgQuality) * 10) / 10 : null,
              avgComm: aggregate.avgComm ? Math.round(Number(aggregate.avgComm) * 10) / 10 : null,
              total: Number(aggregate.total),
  ...
  replyToRating: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existingReply) throw new TRPCError({ code: "CONFLICT", message: "A reply already exists for this review" });
  ...
  listForModeration: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.publishedOnly === true) conditions.push(eq(sanadRatings.isPublished, true));
      if (input.publishedOnly === false) conditions.push(eq(sanadRatings.isPublished, false));
      if (input.officeId) conditions.push(eq(sanadRatings.officeId, input.officeId));
  ...
  getOfficeDashboardStats: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return {
        avgOverall: agg.avgOverall ? Math.round(Number(agg.avgOverall) * 10) / 10 : null,

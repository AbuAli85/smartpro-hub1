/**
 * getDailyStates — read-only daily attendance state resolver (Phase 9A).
 *
  ...
  getDailyStates: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      for (const emp of empRows) {
        // Optional filters
        if (input.employeeId != null && emp.id !== input.employeeId) continue;
        ...
        if (input.siteId != null) {
          const hasSite = rawSchedules.some((s) => s.siteId === input.siteId);
          if (!hasSite) continue;
        }
      ...
      return { date, isHoliday, rows, summary };
    }),
});
